...
export default function History({ completedIncidents, adminTicketsHistory, currentUser, setPage }) {
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState('ALL');

  const isAdmin = currentUser?.role === 'admin';
  const dataList = isAdmin ? (adminTicketsHistory || []) : (completedIncidents || []);
  ...
  const filteredData = dataList.filter((item) => {
    const matchesSearch = 
      ...
      (item.loc && item.loc.toLowerCase().includes(searchQuery.toLowerCase())) ||
  ...
  const getStatusBadge = (status) => {
    ...
    switch (status) {
      case 'PENDING':
        badgeStyle = 'bg-secondary/10 border border-secondary/20 text-secondary shadow-[0_0_10px_rgba(255,138,0,0.05)]';
        dotStyle = 'bg-secondary animate-ping';
        break;
      case 'ASSIGNED':
        badgeStyle = 'bg-primary/10 border border-primary/20 text-primary shadow-[0_0_10px_rgba(0,242,255,0.05)]';
        dotStyle = 'bg-primary animate-pulse';
        break;
      case 'COMPLETED':
        ...
        break;
      case 'CANCELLED':
        badgeStyle = 'bg-error/10 border border-error/20 text-error';
        dotStyle = 'bg-error';
        break;
      default:
        badgeStyle = 'bg-surface-variant border border-outline-variant/30 text-on-surface-variant';
        dotStyle = 'bg-on-surface-variant';
    }

    return (
      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full font-label-caps text-[9px] font-bold ${badgeStyle}`}>
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto w-full flex flex-col gap-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2">
        <div>
          <h1 className="text-4xl font-headline-lg gradient-text font-bold">
            {isAdmin ? 'Logistics Archives' : 'Secure Operations Ledger'}
          </h1>
          <p className="font-body-lg text-body-lg text-on-surface-variant/80 mt-1">
            {isAdmin 
              ? 'Review, audit, and trace all past emergency rescue tickets stored in the database.' 
        ...
        >
          ...
          {isAdmin ? 'Command Portal' : 'Fleet Cockpit'}
        </button>
      </div>
      ...
      <div className="glass-panel rounded-xl p-4 flex flex-col md:flex-row justify-between gap-4 shadow-xl border-outline-variant/15 bg-surface-container-low/20">
        ...
        <div className="relative flex-1">
          ...
          <input 
            type="text" 
            placeholder={isAdmin ? "Search tickets by ID, type, sector, or driver..." : "Search by operation ID, vehicle, or service..."}
            value={searchQuery}
        ...
        {/* Status Filter tabs */}
        <div className="flex gap-2 bg-surface-container-high/40 p-1 rounded-full border border-outline-variant/15 self-start md:self-center overflow-x-auto max-w-full">
          {statusOptions.map((status) => (
            <button
              key={status}
              onClick={() => setStatusFilter(status)}
              className={`px-4 py-1.5 rounded-full font-label-caps text-[10px] font-bold tracking-wider transition-all uppercase whitespace-nowrap ${
                statusFilter === status 
          ...
          <thead>
            {isAdmin ? (
              <tr className="text-on-surface-variant/60 font-label-caps text-label-caps text-[11px] bg-surface-container-low/50 rounded-lg">
                <th className="p-4 font-semibold rounded-l-lg">Incident ID</th>
                <th className="p-4 font-semibold">Date Registered</th>
                <th className="p-4 font-semibold">Rescue Service</th>
                <th className="p-4 font-semibold">Target Sector</th>
                <th className="p-4 font-semibold">Dispatched Unit</th>
                <th className="p-4 font-semibold text-right rounded-r-lg">Operations Status</th>
              </tr>
            ) : (
              <tr className="text-on-surface-variant/60 font-label-caps text-label-caps text-[11px] bg-surface-container-low/50 rounded-lg">
                ...
                >
                  {isAdmin ? (
                    <>
                      ...
                      <td className="p-4 text-on-surface font-semibold">{incident.service}</td>
                      <td className="p-4 text-on-surface-variant">{incident.loc}</td>
                      <td className="p-4 text-on-surface font-medium">{incident.driverName}</td>
                      <td className="p-4 text-right">{getStatusBadge(incident.status)}</td>
                    </>
                  ) : (
                    <>
                      <td className="p-4 text-primary font-label-caps text-label-caps text-xs font-semibold">{incident.id}</td>
                ...
                <td colSpan="6" className="p-12 text-center text-on-surface-variant/60 font-body-sm flex-col gap-3">
                  <div className="flex flex-col items-center justify-center gap-2 py-4">
                    ...
                    <p className="font-semibold text-sm">
                      {isAdmin ? 'No rescue tickets match your filtering criteria.' : 'No operation logs match your search filters.'}
                    </p>
